...
- [ ] **Volume / container as a write target** — `DockerVolumeSource.WriteFiles` returns `ErrReadOnly`. Vol→host promote works (the source's `ReadFiles` handles skill enumeration via `docker.ListVolume`), but host→vol does not. (The picker already disables read-only targets with a badge, so the UX is handled — the remaining gap is the write path itself.) Implement volume writes via a sidecar (`docker run --rm -v <vol>:/data alpine sh -c 'mkdir -p ... && cat > ...'`).
